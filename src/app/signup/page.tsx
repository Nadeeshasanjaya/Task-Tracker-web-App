  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-md flex-col justify-center px-5 py-10">
      <div className="mb-7 text-center">
        ...
        <p className="mb-2 text-xs font-semibold uppercase tracking-[0.18em] text-[var(--coral)]">Start with clarity</p>
        <h1 className="text-3xl font-bold tracking-tight text-[var(--ink)]">Build your focused workspace.</h1>
        <p className="mt-2 text-sm text-[var(--muted)]">A simple home for the work that matters.</p>
      </div>
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 rounded-2xl border border-[var(--line)] bg-[#fbfcfa] p-6 shadow-[0_14px_40px_rgba(23,35,31,0.07)]">
        ...
        {message && <p className="rounded-xl bg-[#e2f3ed] px-3 py-2 text-sm text-[#087f72]">{message}</p>}
        <button
          type="submit"
          disabled={loading}
      ...
      <p className="mt-5 text-center text-sm text-[var(--muted)]">
        ...
        <a href="/login" className="font-semibold text-[var(--teal)] hover:underline">
          Log in
        </a>
      </p>
    </main>
